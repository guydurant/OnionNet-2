...
def PCC_RMSE(y_true, y_pred):
    ...
    # check if pcc is nan
    pcc = tf.where(tf.math.is_nan(pcc), 1.0, pcc)
...
def PCC(y_true, y_pred):
    ...
    pcc = tf.where(tf.math.is_nan(pcc), 0.0, pcc)
    return pcc
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-train_file",
        type=str,
        default="train_features_pKa.csv",
    ...
    parser.add_argument(
        "-valid_file",
        type=str,
        default="valid_features_pKa.csv",
    ...
    bestmodel = tf.keras.callbacks.ModelCheckpoint(
        filepath=args.out, verbose=1, save_best_only=True
    )
